    ...
    <div>
      <form className="row g-3 shadow p-3 mt-3" onSubmit={handleCourseSubmit} style={{ marginBottom: '50px'}}>
        ...
        <div className="col-md-3">
          ...
          <input type="number" className="form-control" id="inputPrice" name='coursePrice' value={newCourse.coursePrice} onChange={/*(e) => setCoursePrice(e.target.value)*/handleCourseChange} style={{ textAlign: 'center' }} />
        </div>
        <div className="col-12">
          <label htmlFor="inputDescription" className="form-label" style={{ fontWeight: 'bold' }}>Description</label>
          <textarea className="form-control" id="inputDescription" rows="3" name='courseDescription' value={newCourse.courseDescription} onChange={/*(e) => setCourseDescription(e.target.value)*/handleCourseChange} style={{ textAlign: 'center' }}></textarea>
        </div>
            ...
            <div className="row g-0">
              ...
              <div className="col-md-9">
                ...
                <div className="position-absolute bottom-0 end-0 p-2">
                  <span className={course.status === 'PENDING' ? `badge bg-warning` : course.status === 'DRAFT' ? 'badge bg-dark' : course.status === 'DECLINED' ? 'badge bg-danger' : course.status === 'APPROVED' ? 'badge bg-success' : 'badge bg-primary'} style={{ fontSize: '15px', paddingInline: '25px', paddingTop: '8px', paddingBottom: '8px', marginRight: '550px' }}>{course.status}</span>
